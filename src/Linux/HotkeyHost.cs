...
internal sealed unsafe partial class HotkeyHost : IHotkeyHost
{
    ...
    private readonly System.Collections.Generic.List<(byte keycode, uint mods, Action callback)> _bindings = new();
    ...
    public void Register(HotkeyModifier mods, uint vk, Action callback)
    {
        ...
        GrabKeyAt(kc, x11Mods | Mod2Mask);
        GrabKeyAt(kc, x11Mods | LockMask | Mod2Mask);
    ...
    private void GrabKeyAt(byte kc, uint mods)
        => XGrabKey(_display, kc, mods, _root, 1, 1, 1);

    private void UngrabKeyAt(byte kc, uint mods) => XUngrabKey(_display, kc, mods, _root);
    ...
    public void Dispose()
    {
        ...
        foreach (var b in _bindings)
        {
            UngrabKeyAt(b.keycode, b.mods);
            UngrabKeyAt(b.keycode, b.mods | LockMask);
            UngrabKeyAt(b.keycode, b.mods | Mod2Mask);
            UngrabKeyAt(b.keycode, b.mods | LockMask | Mod2Mask);
        }
        _bindings.Clear();
